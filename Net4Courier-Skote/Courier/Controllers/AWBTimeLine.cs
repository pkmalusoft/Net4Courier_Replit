...
{
    [SessionExpireFilter]
    public class AWBTimeLineController : Controller
    {
        ...
        {            
            ...
            vm.AWB = new QuickAWBVM();
            vm.MCDetail = new MCDetail();
            vm.PODStatus = new POD();
            vm.PODImage = new tblPodImage();
            vm.PODSignature = new PODSignature();
            //vm.SignatureImage= new iTextSharp.text.Image();
            vm.ManifestDetail = new AWBManifest();
            vm.Details = new List<AWBTrackStatusVM>();
            return View(vm);
        ...
        public JsonResult GetAWBInfo(string awbno)
        {
            ...
            int FyearId = Convert.ToInt32(Session["fyearid"]);
            var list = AWBDAO.GetAWBTimelineInformation(awbno, branchid, FyearId);
            Session["AWBTimeline"] = list;
            if (list != null)
            {
                if (list.AWB.HAWBNo !="")
                {
                    return Json(new { status = "ok", message = "Data Found" }, JsonRequestBehavior.AllowGet);
                }
                else
                {
        ...
        public ActionResult ShowAWBDetails()
        {
             
            AWBTracking vm = (AWBTracking)Session["AWBTimeline"];
            return PartialView("AWBTracking", vm);
        }
