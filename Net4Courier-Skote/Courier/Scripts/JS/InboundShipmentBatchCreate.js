...
function RateProcess() {
    Swal.fire({ title: "Are you sure?", text: "Do you want to Process Courier Charge based on Customer Rate Chart?", icon: "warning", showCancelButton: !0, confirmButtonColor: "#34c38f", cancelButtonColor: "#f46a6a", confirmButtonText: "Yes, Process it!" }).then(
        function (t) {
            if (t.value) {
                $('#spanerror').html('Rate Processing...');
                $('#btnProcessRate').attr('disabled', 'disabled');
                /*$('#loaderpopup').modal('show');*/
                $.ajax({
                    type: 'GET',
                    url: '/Shipment/RateProcess',
                    ...
                    success: function (response) {
                        ...
                        if (response.Status == "OK") {
                            debugger;
                            $('#spanerror').html('Rate Processing Completed!');
                            //$('#loaderpopup').modal('hide');
                            Swal.fire('Save Status', 'Rate Process Completed', 'success');
                            setTimeout(function () {
                                window.location.href = '/Shipment/BatchIndex';
                            },200)
                            
                            //$.ajax({
                            //    type: 'POST',
                            //    url: '/Shipment/ShowBatchShipmentList?ID=' + $('#BATCHID').val(),
                            //    datatype: "html",
                            ...
                            //        $('#btnProcessRate').removeAttr('disabled');
                            //        //$('#TotalAWB').val(max);
                            ...
                            /*$('#loaderpopup').modal('hide');*/
                            $('#btnProcessRate').removeAttr('disabled');
                            Swal.fire("Rate Process Status!", response.Message, "error");
                        }
                    },
                    error: function (err) {
                        $('#loaderpopup').modal('hide');
...
function CostProcess() {
    Swal.fire({ title: "Are you sure?", text: "Do you want to Process Forwarding Cost based on Rate Chart?", icon: "warning", showCancelButton: !0, confirmButtonColor: "#34c38f", cancelButtonColor: "#f46a6a", confirmButtonText: "Yes, Process it!" }).then(
        function (t) {
            if (t.value) {

    $('#spanerror').html('Cost Processing...!');
    $('#btnProcessCost').attr('disabled', 'disabled');
    //$('#loaderpopup').modal('show');
    $.ajax({
        type: 'POST',
        url: '/Shipment/FwdCostProcess',
        data: { BatchID: $('#BATCHID').val() },
        datatype: "json",
        //contentType: false,
        //processData: false,
        success: function (response) {
            ...
            if (response.Status == "OK") {
                debugger;
                $('#spanerror').html('Cost Processing Completed!');
                setTimeout(function () {
                    window.location.href = '/Shipment/BatchIndex';
                }, 200)
                ...
                //        });
                //        $('#loaderpopup').modal('hide');
                //        $('#btnProcessCost').removeAttr('disabled');
                //        //$('#TotalAWB').val(max);
                ...
                $('#spanerror').html(response.Message);
                $('#btnProcessCost').removeAttr('disabled');
                Swal.fire("Cost Process Status!", response.Message, "error");
            }
        },
        error: function (err) {
            $('#loaderpopup').modal('hide');
...
function savemanualfixation() {
    ...
    $.ajax({
        type: 'POST',
        url: '/Shipment/UpdateDataFixation1',
      //  datatype: "html",
...
function savefixation() {
    ...
    for (i = 0; i < maxrow; i++) {
        ...
        if (maxrow == (i + 1)) {
            ...
            $.ajax({
                type: 'POST',
                url: '/Shipment/UpdateDataFixation',
                datatype: "html",
...
function AddAWB(i,t) {
    ...
    var awbentry = {
        ...
        CustomerID: $('#BatchCustomerID').val(),        
    ...
    var html = '<td>' +
        ...
        '<input type="hidden" id="hdnRoute_' + i + '" value="' + awbentry.Route + '" />' +
...
function savetrackstatus() {
    $.ajax({
        type: "POST",
        url: "/Shipment/SaveBatchTrackStatus",
        datatype: "Json",
        data: {
            'BatchID': $('#BATCHID').val(), NewCourierStatusID: $('#NewCourierStatusID').val(),EntryDate:$('#txtChangeDate').val(),Remarks:$('#txtChangeRemarks').val() },
        success: function (response) {
            debugger;
            ...
            if (response.status == "OK") {
                Swal.fire("Save Status!", response.message, "success");
                setTimeout(function () {
                    window.location.href = '/Shipment/BatchCreate?id=' + response.BatchID;
                }, 200)
            } else {
                Swal.fire("Save Status!", response.message, "error");
            }
        ...
        if ((i + 1) == maxrow) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    debugger;
                    if (response.Status == "OK") {
                        if (response.CallPostAPI == "true") {
                            $.ajax({
                                type: 'POST',
                                url: '/Shipment/CallPostAPI',
                                datatype: "json",
                                //contentType: "application/json;charset=UTF-8",
                                //data: {
                                ...
                                success: function (data) {
                                    if (data.Status == "ok") {
                                        ...
                                        Swal.fire("Save Status!", response.message, "success");
                                        setTimeout(function () {
                                            window.location.href = '/Shipment/BatchCreate?id=' + 0;
                                        }, 200)
...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", "#btnImportAPI", function () {
            ...
            $.ajax({
                type: 'POST',
                url: '/Shipment/ImportAPI',
                data: { EntryDate: $('#BatchDate').val(), CustomerID: $('#BatchCustomerID').val() },
                datatype: "html",
                //contentType: false,
                //processData: false,
                success: function (response) {
        ...
        $(document).on("change", "#importFile", function () {
            ...
            formData.append('CustomerID', $('#BatchCustomerID').val());
            if (files.length > 0) {
                $('#btnUpload').attr('disabled', 'disabled');
                $.ajax({
                    url: '/Shipment/ImportFile',
                    data: formData,
                    type: 'POST',
                    contentType: false,
                    processData: false,
