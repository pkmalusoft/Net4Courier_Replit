...
function EditAWB(i) {
    ...
    $('#btnAdd').html('Update');
    setrowactiverev(i);
    $('#InScanID').val($('#hdnShipmentID_' + i).val());
    $('#AWBNo').val($('#hdnAWBNo_' + i).val());
    $('#TransactionDate').val($('#hdnAWBDate_' + i).val());
    $('#PaymentTypeId').val($('#hdnPaymentModeId_'+ i).val()).trigger('change');
    $('#CustomerID').val($('#hdnCustomerID_' + i).val());
    $('#CustomerName').val($('#hdnCustomerName_' + i).val());
    ...
    $('#Shipper').val($('#hdnConsignor_' + i).val());
...
function AddAWB(i,t) {
    ...
    var awbentry = {
        ...
        CargoDescription: $('#ItemDescription').val(),
        Pieces: $('#Pieces').val(),
        Weight: $('#Weight').val(),
...
function SaveAWB() {
    ...
    for (var i = 0; i < maxrow; i++) {
        ...
        if ((i + 1) == maxrow) {
            $.ajax({
                type: "POST",
                url: '/AWBBatchUpload/SaveBatchAWB/',
                datatype: "html",
                data: { BatchID: $('#ID').val(), BatchDate: $('#BatchDate').val(), CustomerID: 0 ,EntrySource:$('#EntrySource').val(), CourierStatusID:$('#CourierStatusID').val(), Details: JSON.stringify(items),DeleteDetails:JSON.stringify(deleteditems) },
                success: function (response) {
                    debugger;
                    if (response.Status == "OK") {
                        ...
                        Swal.fire("Save Status!", response.message, "success");                        
                        setTimeout(function () {
                            window.location.href = '/AWBBatchUpload/BatchCreate?id=' + response.BatchID;
                        },200)
...
(function ($) {
    ...
    function initformControl() {
        ...
        $("#Consignee").autocomplete({
            source: function (request, response) {
                $.ajax({
                    ...
                    datatype: "json",
                    data: {
                        term: request.term, Shipper: $('#Shipper').val(), ShowAll: $('#ShowAllConsignee').prop('checked')
                    },
                    success: function (data) {
                        response($.map(data, function (val, item) {
                            return {
        ...
        if ($('#ID').val() > 0) {
            ...
            $('#btnAutoFixation').addClass('hide');
            $('#btnFixation').addClass('hide');
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.Status === 1) {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: '/AWBBatchUpload/ShowShipmentList',
                                datatype: "html",
                                success: function (data) {
                                    $('#loaderpopup').modal('hide');
                                    $('#EntrySource').val('EXL');
