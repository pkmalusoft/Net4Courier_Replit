    ...
    $(document).ready(function () {
        ...
        $(document).on("change", "#importFile", function () {
            ...
            if (files.length > 0) {
                ...
                $.ajax({
                    url: '/CustomerRatesMaster/ImportFile',
                    data: formData,
                    type: 'POST',
                    contentType: false,
                    processData: false,
                    success: function (response) {
                        if (response.Status === 1) {
                            ...
                            $.each(response.data, function (index, item) {
                                ...
                                var html = '<tr><td><input type="hidden" name="CustRateDetails[' + index + '].CustomerRateDetID" value ="0" id = "hdnCustomerRateDetID_' + index + '" /><input class="hdndeleted" type="hidden" name="CustRateDetails[' + index + '].Deleted" value = "false" id = "hdndeleted_' + index + '" /><input type="text" class="text-right" id="txtWtFrom_' + index + '" value="' + _AddWtFrom + '" name="CustRateDetails[' + index + '].AddWtFrom" /></td>' +
                                    '<td><input type="text" class="text-right" id="txtWtTo_' + index + '"  value="' + _AddWtTo + '" name="CustRateDetails[' + index + '].AddWtTo"></td>' +
                                    '<td><input type="text"  class="text-right" id="txtIncrWt_' + index + '" value="' + _IncrWt + '" name="CustRateDetails[' + index + '].IncrWt"></td>' +
                                    '<td><input type="text" class="text-right"  id="txtContractRate_' + index + '" value="' + _ContractRate + '" name="CustRateDetails[' + index + '].ContractRate" onchange = "contractdatecal(' + index + ')" ></td>' +
                                    '<td><input type="text"  class="text-right crate" id="txtAddRate_' + index + '" value="' + _AddRate + '" name="CustRateDetails[' + index + '].AddRate"></td>' +
                                    //'<td><a href="javascript:void(0)" onclick="check(this)" value="Click"><i class="fa fa-pencil"></i></a></td>' +
                                    '<td><a  class="text-danger" href="javascript:void(0)" onclick="deletetrans(this,' + index + ')"><i class="mdi mdi-delete font-size-18"></i></a></td>' +
                                    +'</tr>';
