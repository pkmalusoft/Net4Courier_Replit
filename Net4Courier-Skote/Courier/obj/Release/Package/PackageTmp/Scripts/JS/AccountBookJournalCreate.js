...
function CalculateTotal(index, type) {
    ...
    for (i = 0; i < maxrow; i++) {
        ...
        $('#spandebittotal').html(parseFloat(TotalDebit).toFixed(2));
        $('#spancredittotal').html(parseFloat(TotalCredit).toFixed(2));
        $('#spancredittotal1').html('');
        $('#spandebittotal1').html('');
        var diff = parseFloat(TotalDebit) - parseFloat(TotalCredit);
        if (parseFloat(diff) > 0) {
            $('#spancredittotal1').html(parseFloat(diff).toFixed(2));
            $('#spandebittotal1').html('');
        }
        else if (parseFloat(diff) < 0){
            $('#spandebittotal1').html((-1 * parseFloat(diff)).toFixed(2));
            $('#spancredittotal1').html('');
        }
    }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == "OK") {
                        ...
                        window.location.href = '/Accounts/IndexAcJournal';
