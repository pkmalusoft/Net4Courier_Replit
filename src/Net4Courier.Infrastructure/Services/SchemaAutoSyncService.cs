...
public class SchemaAutoSyncService
{
    ...
    public async Task<SchemaSyncResult> PreviewSchemaChangesAsync(DbContext dbContext, CancellationToken cancellationToken = default)
    {
        var result = new SchemaSyncResult();
        try
        {
            _logger.LogInformation("Starting schema change preview (dry-run mode)...");
            ...
            foreach (var entityType in model.GetEntityTypes())
            {
                ...
                if (!tableExists)
                {
                    result.DetectedMissing.Add($"Table: {tableName} (will be created)");
                    result.TablesCreated++;
                }
                else
                {
            ...
            result.Success = true;
            if (result.DetectedMissing.Count == 0)
            {
                result.Changes.Add("Schema is up to date. No changes needed.");
            }
            _logger.LogInformation("Schema preview completed. {TableCount} new tables, {ColumnCount} new columns detected.",
                result.TablesCreated, result.ColumnsAdded);
            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Schema preview failed.");
            result.Success = false;
