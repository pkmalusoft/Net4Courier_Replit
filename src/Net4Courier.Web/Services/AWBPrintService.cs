...
public class AWBPrintService
{
    ...
    private void A5Header(ColumnDescriptor column, InscanMaster shipment, string companyName, byte[]? logoData)
    {
        ...
        column.Item().BorderBottom(2).PaddingBottom(5).Row(row =>
        {
            row.RelativeItem(3).Column(left =>
            {
                ...
                {
                    left.Item().Height(26).Image(effectiveLogo).FitHeight();
                }
                else
                {
                ...
                left.Item().PaddingTop(2).Row(tagRow =>
                {
                    tagRow.AutoItem().Background("#1e3a5f").PaddingHorizontal(6).PaddingVertical(2)
                        .Text(serviceDesc.ToUpper()).FontColor(Colors.White).Bold().FontSize(6);
                });
            });
            ...
            row.RelativeItem(3).AlignRight().Column(right =>
            {
                ...
                right.Item().AlignRight().Text(shipment.AWBNo ?? "").Bold().FontSize(20).LetterSpacing(0.1f);
            });
        });
    }
    ...
    {
        column.Item().PaddingTop(4).PaddingBottom(5).Row(row =>
        {
            row.RelativeItem().Text(text =>
            {
                text.Span("Account No  ").Bold().FontSize(7).FontColor(Colors.Grey.Darken1);
                text.Span(shipment.CustomerId?.ToString() ?? "").Bold().FontSize(8);
            });
            row.RelativeItem().AlignCenter().Text(text =>
            {
                text.Span("Booking Date : ").Bold().FontSize(7).FontColor(Colors.Grey.Darken1);
                text.Span(shipment.TransactionDate.ToString("dd/MM/yyyy")).Bold().FontSize(8);
            });
            row.RelativeItem().AlignRight().Text(text =>
            {
                text.Span("Shipper's Reference  ").Bold().FontSize(7).FontColor(Colors.Grey.Darken1);
                text.Span(shipment.ReferenceNo ?? "").FontSize(8);
            });
        });
    }
    ...
    {
        column.Item().PaddingTop(4).Row(mainRow =>
        {
            mainRow.RelativeItem(6).PaddingRight(8).Column(leftCol =>
            {
                ...
                leftCol.Item().PaddingTop(5).Border(1).Background("#fffbeb").Padding(4).Row(instrRow =>
                {
                    instrRow.ConstantItem(130).Text("Item Description / Special Instruction:").Bold().FontSize(7);
                    instrRow.RelativeItem().Text(itemDesc).FontSize(7).Italic();
                });
            });
    ...
    private void A5ShipperCard(ColumnDescriptor card, InscanMaster shipment)
    {
        ...
        card.Item().Padding(6).Column(content =>
        {
            ...
            content.Item().PaddingTop(2).Text(address).FontSize(8).FontColor(Colors.Grey.Darken2);
            content.Item().PaddingTop(5).Text(text =>
            {
                text.Span("Tel: ").FontSize(7).FontColor(Colors.Grey.Darken1);
                text.Span(shipment.ConsignorPhone ?? "").Bold().FontSize(8);
                if (!string.IsNullOrWhiteSpace(shipment.ConsignorMobile))
                {
                    text.Span("   Mob: ").FontSize(7).FontColor(Colors.Grey.Darken1);
                    text.Span(shipment.ConsignorMobile).Bold().FontSize(8);
                }
            });
        });
    }
    ...
    private void A5ReceiverCard(ColumnDescriptor card, InscanMaster shipment)
    {
        ...
        card.Item().Padding(6).Column(content =>
        {
            ...
            {
                text.Span("Tel: ").FontSize(7).FontColor(Colors.Grey.Darken1);
                text.Span(shipment.ConsigneePhone ?? "").Bold().FontSize(8);
                if (!string.IsNullOrWhiteSpace(shipment.ConsigneeMobile))
                {
                    text.Span("   Mob: ").FontSize(7).FontColor(Colors.Grey.Darken1);
                    text.Span(shipment.ConsigneeMobile).Bold().FontSize(8);
                }
            });
        });
    }
    ...
    private void A5BarcodeBox(ColumnDescriptor col, InscanMaster shipment)
    {
        col.Item().Border(1).Padding(5).AlignCenter().Column(bc =>
        {
            ...
            {
                bc.Item().AlignCenter().Height(36).Image(shipment.BarcodeImage);
            }
            bc.Item().PaddingTop(3).AlignCenter().Text(shipment.AWBNo ?? "").Bold().FontSize(9).LetterSpacing(0.4f);
        });
    }
    ...
    private void A5WeightPiecesBox(ColumnDescriptor col, InscanMaster shipment)
    {
        ...
        col.Item().PaddingTop(4).Border(1).Row(row =>
        {
            ...
            {
                c.Item().AlignCenter().Text("V.WEIGHT").Bold().FontSize(6).FontColor(Colors.Grey.Darken1);
    ...
    private void A5CollectCashBox(ColumnDescriptor col, InscanMaster shipment)
    {
        ...
        col.Item().PaddingTop(4).Background(Colors.Black).Padding(8).AlignCenter().Column(cash =>
        {
            cash.Item().AlignCenter().Text("COLLECT CASH").FontColor(Colors.White).Bold().FontSize(8).LetterSpacing(0.15f);
            cash.Item().PaddingTop(3).AlignCenter().Row(amtRow =>
            {
                amtRow.AutoItem().AlignBottom().PaddingBottom(3).Text(currency).FontColor(Colors.White).Bold().FontSize(10);
                amtRow.AutoItem().PaddingLeft(4).Text(totalAmount > 0 ? totalAmount.ToString("N2") : "0.00").FontColor(Colors.White).Bold().FontSize(26);
            });
        });
    }
    ...
    private void A5PodFooter(ColumnDescriptor column, InscanMaster shipment, string? website = null)
    {
        ...
        column.Item().PaddingTop(6).BorderTop(2).PaddingTop(4).Column(footer =>
        {
            footer.Item().Row(podHeaderRow =>
            {
                ...
                {
                    leftPod.Item().Text("Item Description / Special Instruction").Bold().FontSize(7).FontColor(Colors.Grey.Darken1);
                    leftPod.Item().PaddingTop(2).Text(shipment.Remarks ?? "").FontSize(7);
                });
                podHeaderRow.RelativeItem().AlignRight().Column(rightPod =>
                {
                    rightPod.Item().AlignRight().Text("Proof of Delivery").Bold().FontSize(8);
                    rightPod.Item().PaddingTop(2).AlignRight().Text("Consignment Received in Good Condition").FontSize(6).FontColor(Colors.Grey.Darken1);
                });
            });

            footer.Item().PaddingTop(5).Row(podRow =>
            {
                podRow.RelativeItem().PaddingRight(12).Column(colBy =>
                {
                    colBy.Item().Text("Collected By (Name & Signature)").FontSize(6).FontColor(Colors.Grey.Darken1);
                    colBy.Item().PaddingTop(12).BorderBottom(0.5f);
                    colBy.Item().PaddingTop(3).Text(text =>
                    {
                        text.Span("Service Type : ").FontSize(6).FontColor(Colors.Grey.Darken1);
                        text.Span(serviceType.ToUpper()).Bold().FontSize(7);
                    });
                    colBy.Item().PaddingTop(2).Text("Date & Time").FontSize(6).FontColor(Colors.Grey.Darken1);
                    colBy.Item().PaddingTop(6).BorderBottom(0.5f);
                });
                podRow.RelativeItem().Column(delBy =>
                {
                    delBy.Item().Text("Name & Signature").FontSize(6).FontColor(Colors.Grey.Darken1);
                    delBy.Item().PaddingTop(12).BorderBottom(0.5f);
                    delBy.Item().PaddingTop(3).Text("Date & Time").FontSize(6).FontColor(Colors.Grey.Darken1);
                    delBy.Item().PaddingTop(6).BorderBottom(0.5f);
                    delBy.Item().PaddingTop(3).Text("Delivered By (Name & Signature)").FontSize(6).FontColor(Colors.Grey.Darken1);
                    delBy.Item().PaddingTop(6).BorderBottom(0.5f);
                    delBy.Item().PaddingTop(2).Text("Date & Time").FontSize(6).FontColor(Colors.Grey.Darken1);
                });
            });

            footer.Item().PaddingTop(5).Row(bottomRow =>
            {
                bottomRow.RelativeItem().Text(text =>
                {
                    ...
                    {
                        text.Span(website).FontSize(7).FontColor("#1e3a5f");
                    }
                });
                bottomRow.ConstantItem(50).AlignRight().Text("Page 1/1").Bold().FontSize(7).FontColor(Colors.Grey.Medium);
            });
        });
    }
    ...
    private void LabelHeader(ColumnDescriptor column, InscanMaster shipment)
    {
        ...
        column.Item().BorderBottom(2).PaddingHorizontal(8).PaddingVertical(6).Row(row =>
        {
            row.RelativeItem().Column(c =>
            {
                c.Item().Text(movementLabel).Bold().FontSize(14);
                c.Item().Text(shipment.CargoDescription ?? "E-COMMERCE DELIVERY").Bold().FontSize(7);
            });
            row.ConstantItem(60).AlignRight().Column(c =>
            {
                c.Item().AlignRight().Text("DATE").Bold().FontSize(7);
                c.Item().AlignRight().Text(shipment.TransactionDate.ToString("dd/MM/yyyy")).Bold().FontSize(9);
            });
        });
    }
    ...
    {
        column.Item().BorderBottom(2).PaddingVertical(4).AlignCenter().Column(c =>
        {
            if (shipment.BarcodeImage != null)
            {
                c.Item().AlignCenter().MaxHeight(50).MaxWidth(250).Image(shipment.BarcodeImage).FitArea();
            }
            c.Item().AlignCenter().Text(shipment.AWBNo).Bold().FontSize(14).LetterSpacing(0.15f);
        });
    }

    private void LabelShipper(ColumnDescriptor column, InscanMaster shipment)
    {
        column.Item().BorderBottom(1).Background("#f9fafb").PaddingHorizontal(6).PaddingVertical(4).Column(c =>
        {
            c.Item().Text("SHIPPER").Bold().FontSize(7);
            c.Item().Height(2);
            c.Item().Row(row =>
            {
                row.RelativeItem().Column(sc =>
                {
                    sc.Item().Text(Truncate(shipment.Consignor, 40)).Bold().FontSize(8);
                    var shipperAddr = $"{shipment.ConsignorAddress1} {shipment.ConsignorAddress2}".Trim();
                    if (!string.IsNullOrWhiteSpace(shipperAddr))
                        sc.Item().Text(Truncate(shipperAddr, 50)).FontSize(7);
                });
                row.ConstantItem(80).AlignRight().Text(CombinePhoneNumbers(shipment.ConsignorPhone, shipment.ConsignorMobile)).Bold().FontSize(7);
            });
        });
    }
    ...
    {
        column.Item().BorderBottom(2).PaddingHorizontal(8).PaddingVertical(6).Column(c =>
        {
            c.Item().Background(Colors.Black).PaddingHorizontal(6).PaddingVertical(2)
                .Text("Deliver To").FontColor(Colors.White).Bold().FontSize(8);
            c.Item().Height(4);
            c.Item().Text(Truncate(shipment.Consignee, 40)).Bold().FontSize(14);

            var fullAddr = $"{shipment.ConsigneeAddress1} {shipment.ConsigneeAddress2}".Trim();
            if (!string.IsNullOrWhiteSpace(fullAddr))
                c.Item().Text(Truncate(fullAddr, 80)).FontSize(9);
            ...
            c.Item().Row(row =>
            {
                ...
                {
                    lc.Item().Text(shipment.ConsigneeCity ?? "").Bold().FontSize(20);
                    lc.Item().Height(2);
                ...
                row.ConstantItem(50).AlignRight().Column(rc =>
                {
                    ...
                    rc.Item().AlignRight().Text(GetPortCode(shipment.OriginPortCode, shipment.ConsignorCity)).Bold().FontSize(16);
                });
            });
        });
    }
    ...
    private void LabelMetrics(ColumnDescriptor column, InscanMaster shipment)
    {
        ...
        column.Item().BorderBottom(2).Background("#f9fafb").Height(42).Row(row =>
        {
            row.RelativeItem().BorderRight(1).PaddingVertical(4).AlignCenter().AlignMiddle().Column(c =>
            {
                c.Item().AlignCenter().Text("WEIGHT").Bold().FontSize(6);
                c.Item().AlignCenter().Text($"{shipment.Weight?.ToString("F1") ?? "0.0"} KG").Bold().FontSize(10);
            });
            row.RelativeItem().BorderRight(1).PaddingVertical(4).AlignCenter().AlignMiddle().Column(c =>
            {
                c.Item().AlignCenter().Text("PIECES").Bold().FontSize(6);
                c.Item().AlignCenter().Text($"{shipment.Pieces ?? 1} of {shipment.Pieces ?? 1}").Bold().FontSize(10);
            });
            row.RelativeItem().BorderRight(1).PaddingVertical(4).AlignCenter().AlignMiddle().Column(c =>
            {
                c.Item().AlignCenter().Text("INCO TERMS").Bold().FontSize(6);
                c.Item().AlignCenter().Text(paymentMode).Bold().FontSize(10).FontColor("#1565c0");
            });
            row.RelativeItem().PaddingVertical(4).PaddingHorizontal(2).AlignCenter().AlignMiddle().Column(c =>
            {
    ...
    private void LabelFooter(ColumnDescriptor column, InscanMaster shipment)
    {
        ...
        column.Item().Background(Colors.Black).PaddingHorizontal(8).PaddingVertical(6).Column(c =>
        {
            c.Item().Row(row =>
            {
                row.RelativeItem().AlignMiddle().Text("Collect Payment").FontColor(Colors.White).Bold().FontSize(9);
                row.ConstantItem(100).Column(bd =>
                {
                    bd.Item().BorderBottom(0.5f).BorderColor("#4b5563").PaddingBottom(2).Row(r =>
                    {
            ...
            c.Item().Height(4);
            c.Item().BorderTop(0.5f).BorderColor("#4b5563").PaddingTop(4).Row(row =>
            {
                ...
                {
                    tc.Item().AlignRight().Text($"TOTAL AMOUNT ({currency})").FontColor(Colors.White).Bold().FontSize(7);
                    tc.Item().AlignRight().Text(totalAmount > 0 ? totalAmount.ToString("N2") : "0.00").FontColor(Colors.White).Bold().FontSize(22);
                });
            });
        });
    }
